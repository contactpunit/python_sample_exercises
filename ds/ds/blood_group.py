...
def check_bt(donor, recipient):
    ...
    """
    print(type(donor))
    if not isinstance(donor, (str, int, Enum)) or not isinstance(recipient, (str, int, Enum)):
        raise TypeError()
    elif isinstance(donor, int) and (donor > 7 or donor < 0):
        raise ValueError()
    elif isinstance(recipient, int) and (recipient > 7 or recipient < 0):
        raise ValueError()
    elif isinstance(donor, str) and donor not in blood_type_text:
        raise ValueError()
    elif isinstance(recipient, str) and recipient not in blood_type_text:
        raise ValueError()
    elif isinstance(donor, Enum) and donor not in [b for b in Bloodtype]:
        raise ValueError()
    elif isinstance(recipient, Enum) and recipient not in [b for b in Bloodtype]:
        raise ValueError()
    if isinstance(donor, (str, Enum)):
        if isinstance(donor, str):
            donor = blood_type_text[donor].value
        else:
            donor = donor.value
    if isinstance(recipient, (str, Enum)):
        if isinstance(recipient, str):
            recipient = blood_type_text[recipient].value
        else:
            recipient = recipient.value
    result = _particular_antigen_comp(donor, recipient)
...
print(check_bt(8, Bloodtype.A_POS))
